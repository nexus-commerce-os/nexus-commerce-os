...
- **Started:** 2026-07-13
- **Current phase:** `P0.2 — Core Platform` (Identity bounded context; Phase-0 documentation ratified, P0.1 scaffold delivered but its live exit gate still unverified). Per-increment status: [PHASE-P0.2-STATUS.md](PHASE-P0.2-STATUS.md)
...
> **CURRENT STATE (2026-07-31) — P0.2 Identity: I-7e ACCEPTED, production-verification PENDING.** CTO review of the I-7e outbound email notification adapter returned **CONDITIONALLY ACCEPTED**, and introduced a binding distinction the board must now carry per increment: **Architecture COMPLETE · Implementation COMPLETE · Production Verification PENDING** — "implementation complete" may no longer be read as "production verified". Confirmed at review: domain unchanged since I-1 · Ports→Adapters preserved · no vendor leakage (nodemailer confined to one factory behind a 1-method transport interface) · no secret on domain events (the raw link token is deliberately absent from `EmailVerificationRequested`/`PasswordResetRequested`; it travels use-case result → orchestrator → port only) · anti-enumeration preserved (`SendPasswordReset` always returns ok and reports delivery failure out-of-band, since a mail error can only occur for an address that exists) · failed delivery does not corrupt domain state (the token is already committed and stays valid; asserted by tests that reuse the link after a failure). Evidence: commit `e2ac278`, CI run `30596992219`, **`ci-gate` SUCCESS**, **404 tests passed / 0 skipped in CI with `NEXUS_PG_TESTS=1`** (the Postgres-container suites genuinely ran — checked in the job log, not inferred from the green tick), typecheck 9/9, lint 0 errors, build 5/5, prettier clean. **Three evidence gaps explicitly tracked, NOT to be implemented now:** **FU-1 real SMTP integration `NOT VERIFIED`** — no message has ever traversed a real server, so `createSmtpTransport` has zero test coverage (STARTTLS negotiation, `requireTLS` failing closed, link rendering, UTF-8, Gmail/SES/Mailgun/Postmark all unproven); **FU-2 end-to-end `NOT VERIFIED`** — blocked on I-7b, nothing calls the orchestrators in production yet; **FU-3 operational readiness** — retry/backoff, outbox, dead-letter, delivery metrics, provider health, resend rate-limiting, all deliberately out of scope. Recorded in [PHASE-P0.2-STATUS.md](PHASE-P0.2-STATUS.md) (repo root, not `docs/` — the [ADR-0023](docs/adr/ADR-0023-doc-freeze-and-approval-gate.md) freeze is untouched and no ADR changed). **P0.2 exit gate NOT MET:** of its six pillars only Identity is built — RBAC, Organization, Settings, Feature Flags and Audit Logs are NOT STARTED. **Next: I-7b, gated on two unresolved architectural rulings** — (1) session ownership vs. the managed IdP (doc 07 §387 says one managed IdP issues access tokens; the I-2 sessions may instead be the BFF session store), (2) the ADR-0020 canonical contract source, which must generate OpenAPI/protobuf/GraphQL from one source that does not yet exist. **RBAC, Organization, Audit, Feature Flags, Commerce and AI remain explicitly unauthorized.**

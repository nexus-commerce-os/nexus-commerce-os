...
```mermaid
flowchart LR
    CERT[Certified docs 01-13 + ADRs 0001-0023<br/>FROZEN: what and why] --> E1[E1 this spec<br/>how to build P0.1]
...
[`.github/CODEOWNERS`](../../.github/CODEOWNERS) assigns an **explicit owning team to every path** in the monorepo. The wildcard default (`* @nexus-commerce-os/platform`) guarantees there is no unowned code; more specific rules layer domain ownership on top. Because branch protection requires CODEOWNERS review, the ownership map is not documentation — it is the enforced approval routing for every PR.

### 5.2 The team map

| Path | Owning team(s) | Rationale |
|------|----------------|-----------|
| `*` (default) | `@nexus-commerce-os/platform` | Anything not otherwise claimed |
| `/infrastructure/terraform/` | `@nexus-commerce-os/infrastructure` `@nexus-commerce-os/cloud-security` | Cloud estate changes need both infra and security eyes |
| `/infrastructure/kubernetes/` | `@nexus-commerce-os/platform` `@nexus-commerce-os/sre` | Workload runtime + reliability |
| `/infrastructure/monitoring/` | `@nexus-commerce-os/sre` | Observability is SRE-owned |
| `/infrastructure/security/` | `@nexus-commerce-os/cloud-security` | Policy-as-code, supply chain, IAM |
| `/infrastructure/github/` | `@nexus-commerce-os/devsecops` | Repo governance, OIDC trust |
| `/.github/` | `@nexus-commerce-os/devsecops` `@nexus-commerce-os/platform` | Pipelines + governance |
| `/apps/` | `@nexus-commerce-os/frontend` | User-facing surfaces |
| `/services/auth/` | `@nexus-commerce-os/platform` `@nexus-commerce-os/cloud-security` | Identity is security-sensitive |
| `/services/{catalog,affiliate,search}/` | `@nexus-commerce-os/commerce` | Commerce domain |
| `/services/ai/` | `@nexus-commerce-os/ai` | AI domain |
| `/services/analytics/` | `@nexus-commerce-os/data` | Data/analytics domain |
| `/services/platform-hello/` | `@nexus-commerce-os/platform` | The platform canary |
| `/packages/` | `@nexus-commerce-os/platform` `@nexus-commerce-os/frontend` | Shared libraries |
| `/docs/` | `@nexus-commerce-os/architecture` | **Certified + frozen** — changes require architecture review |
| `/tests/` | `@nexus-commerce-os/sre` `@nexus-commerce-os/devsecops` | Smoke + policy tests |
...
- **`/docs/` is owned by `@nexus-commerce-os/architecture`.** The certified corpus is frozen; a docs change is an architecture-review event, which is exactly why the doc-consistency lint (§6) guards against ADR↔doc drift as a merge gate.
- **Feature teams are added at their phase.** P0.1 owners are the platform/scaffold owners; a domain team (e.g. `@nexus-commerce-os/commerce` deepening) is layered on when its sub-phase begins ([13](../13-implementation-roadmap.md)).

### 5.4 Bootstrap dependency

The `@nexus-commerce-os/*` teams are **placeholders** until the GitHub org is created at P0.1 bootstrap. Branch protection, CODEOWNERS review, and prod environment approvers all depend on these teams existing (Risk R-3, [P0.1 report](../../PHASE-P0.1-REPORT.md)). Creating the org and teams is step 1 of the exit-gate runbook (§11.4).
...
Security is **built-in from the first commit**, not a later phase. The controls below fail closed at each link; the chain is owned by `@nexus-commerce-os/cloud-security` and codified in [`infrastructure/security`](../../infrastructure/security/README.md).
...
1. The **GitHub org + `@nexus-commerce-os/*` teams** exist; branch protection + environments are applied from [`infrastructure/github`](../../infrastructure/github/README.md).
...
| R-3 | `@nexus-commerce-os/*` GitHub teams — **RESOLVED 2026-07-20** | Medium | ✅ Org `nexus-commerce-os` (id 306595611) + 10 teams created with repo write access (Stage A); CODEOWNERS resolves + auto-requests owners |
